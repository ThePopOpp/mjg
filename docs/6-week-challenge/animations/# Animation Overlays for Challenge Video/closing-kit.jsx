...
/* you finished */
function BFinished({ T, t0 }) {
  const a = clamp(MOTION.enter(t0 + 0.3, 0.9)(T), 0, 1);
  const b = clamp(MOTION.enter(t0 + 1.9, 0.9)(T), 0, 1);
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Scrim o={MOTION.enter(t0, 0.6)(T)} />
      <div style={{ position: 'absolute', left: 140, bottom: 182, width: 1560 }}>
        <div style={{ fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 116, color: C.white, ...rise(a, 30) }}>You finished.</div>
        <div style={{ width: 420 * clamp(MOTION.draw(t0 + 1.3, 0.8)(T), 0, 1), height: 2, background: C.gold, margin: '28px 0' }} />
        <div style={{ fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 60, color: 'rgba(244,241,234,0.72)', ...rise(b, 24) }}>
          Six weeks of work most men <em style={{ fontStyle: 'italic', color: C.goldLite, whiteSpace: 'nowrap' }}>never slow down long enough to do.</em>
        </div>
      </div>
    </div>
  );
...
/* a blueprint was never meant to sit in a drawer */
function BDrawer({ T, t0 }) {
  const a = clamp(MOTION.enter(t0 + 0.3, 0.9)(T), 0, 1);
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 200, right: 200, top: 0, bottom: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={{ fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 74, color: 'rgba(244,241,234,0.66)', ...rise(a, 26) }}>
          A blueprint was never meant to sit in a drawer.
        </div>
        <div style={{ width: 1520 * clamp(MOTION.draw(t0 + 1.7, 0.9)(T), 0, 1), height: 2, background: C.gold, margin: '44px 0' }} />
        <div style={{ fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 92, color: C.white, lineHeight: 1.12, ...rise(b, 28) }}>
          The question is not “did I finish?” It is <em style={{ fontStyle: 'italic', color: C.goldLite, whiteSpace: 'nowrap' }}>“how do I keep going?”</em>
        </div>
      </div>
    </div>
  );
...
/* ON SCREEN: KEEP BUILDING */
function BKeepBuilding({ T, t0 }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <Monogram h={54} color={C.gold} style={{ opacity: p, marginBottom: 44 }} />
        <div style={{ fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 168, lineHeight: 1, color: C.white, ...rise(p, 40) }}>Keep Building</div>
        <div style={{ width: 640 * slab, height: 14, background: C.gold, margin: '30px 0 34px' }} />
        <div style={{ ...caps(30, C.cream, 8), opacity: clamp(MOTION.enter(t0 + 1.1, 0.9)(T), 0, 1) }}>Your next step</div>
      </div>
    </div>
  );
}

/* three next steps — numbered, sequential */
const STEPS = [
  ['Keep the accountability alive', 'Stay connected to one man. Protect your thirty-day commitment. Review your Blueprint in ninety days.'],
  ['Take the framework into daily life', 'ForgedLife — habit tracking, challenges, prayer, and men helping each other follow through.'],
  ['Lead the next group', 'You do not need to be an expert. You just walked through it. Invite one man and multiply it.'],
];
function BSteps({ T, t0 }) {
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Dim o={0.93 * clamp(MOTION.enter(t0, 0.7)(T), 0, 1)} color={C.ink} />
      <div style={{ position: 'absolute', left: 140, top: 160 }}>
        <Kicker text="Three clear next steps" p={MOTION.enter(t0 + 0.15, 0.7)(T)} />
      </div>
      <div style={{ position: 'absolute', left: 140, right: 140, top: 290 }}>
        {STEPS.map(([h, sub], i) => {
          const p = clamp(MOTION.enter(t0 + 0.7 + i * 1.8, 0.75)(T), 0, 1);
          return (
            <div key={i} style={{ display: 'flex', gap: 44, alignItems: 'flex-start', paddingBottom: 34, marginBottom: 34, borderBottom: '1px solid rgba(244,241,234,' + 0.2 * p + ')', ...rise(p, 26) }}>
              <span style={{ fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 58, color: C.gold, width: 96 }}>{'0' + (i + 1)}</span>
              <span>
                <span style={{ display: 'block', fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 58, color: C.white, lineHeight: 1.08 }}>{h}</span>
                <span style={{ display: 'block', ...caps(21, 'rgba(244,241,234,0.7)', 2.8), marginTop: 14, lineHeight: 1.6 }}>{sub}</span>
              </span>
            </div>
          );
        })}
...
/* four-pillar pulse stays visible */
function BPulseKeep({ T, t0 }) {
  const pillars = ['Family', 'Fitness', 'Fun', 'Finances'];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Scrim o={MOTION.enter(t0, 0.6)(T)} />
      <div style={{ position: 'absolute', left: 140, right: 140, bottom: 168 }}>
        <div style={{ fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 70, color: C.white, ...rise(MOTION.enter(t0 + 0.2, 0.8)(T), 22) }}>
          Seasons change which pillar leads. <em style={{ fontStyle: 'italic', color: C.goldLite, whiteSpace: 'nowrap' }}>All four stay visible.</em>
        </div>
        <div style={{ display: 'flex', gap: 26, marginTop: 44, alignItems: 'flex-end' }}>
          {pillars.map((s, i) => {
            const p = clamp(MOTION.pop(t0 + 0.9 + i * 0.35, 0.7)(T), 0, 1);
            const h = [150, 210, 130, 180][i];
            return (
              <div key={i} style={{ flex: 1 }}>
                <div style={{ height: h * p, background: 'rgba(244,241,234,0.88)' }} />
                <div style={{ ...caps(25, C.cream, 3), marginTop: 18, textAlign: 'center' }}>{s}</div>
              </div>
            );
          })}
        </div>
...
/* ON SCREEN: theforged.life • MichaelJGauthier.com */
function BLinks({ T, t0, tw }) {
  const a = clamp(MOTION.enter(t0 + 0.3, 0.9)(T), 0, 1);
  const b = clamp(MOTION.enter(t0 + 1.0, 0.9)(T), 0, 1);
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Dim o={0.95 * clamp(MOTION.enter(t0, 0.7)(T), 0, 1)} color={C.ink} />
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 46 }}>
        <div style={{ ...caps(24, C.gold, 10), opacity: a }}>Where to go from here</div>
        <div style={{ fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 108, color: C.white, ...rise(a, 30) }}>{tw.appUrl}</div>
        <div style={{ width: 200 * clamp(MOTION.draw(t0 + 0.9, 0.8)(T), 0, 1), height: 2, background: C.gold }} />
        <div style={{ fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 84, color: 'rgba(244,241,234,0.82)', ...rise(b, 26) }}>{tw.url}</div>
      </div>
    </div>
  );
}
...
/* it overflows */
function BOverflow({ T, t0 }) {
  const items = ['Your family', 'Your friends', 'Your church', 'Your community', 'The next man'];
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <Dim o={0.92 * clamp(MOTION.enter(t0, 0.7)(T), 0, 1)} color={C.ink} />
      <div style={{ position: 'absolute', left: 140, right: 140, top: 230 }}>
        <div style={{ fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 84, color: C.white, ...rise(MOTION.enter(t0 + 0.2, 0.9)(T), 26) }}>
          That is what a well-built life does. It <em style={{ fontStyle: 'italic', color: C.goldLite, whiteSpace: 'nowrap' }}>overflows.</em>
        </div>
      </div>
      <div style={{ position: 'absolute', left: 140, right: 140, top: 520, display: 'flex', gap: 20 }}>
        {items.map((s, i) => {
          const p = clamp(MOTION.enter(t0 + 1.0 + i * 0.5, 0.7)(T), 0, 1);
          return (
            <div key={i} style={{ flex: 1, ...rise(p, 28) }}>
              <div style={{ height: 3, background: C.gold, opacity: 1 - i * 0.14, transform: 'scaleX(' + p + ')', transformOrigin: 'left' }} />
              <div style={{ ...caps(26, C.cream, 3), marginTop: 22, opacity: 1 - i * 0.1 }}>{s}</div>
            </div>
          );
        })}
      </div>
      <div style={{ position: 'absolute', left: 140, right: 140, top: 760, ...caps(23, 'rgba(244,241,234,0.6)', 4), opacity: clamp(MOTION.enter(t0 + 3.8, 0.9)(T), 0, 1), lineHeight: 1.6 }}>
        …who is quietly drifting and does not know there is another way
      </div>
    </div>
  );
}
...
/* final series card */
function BSeriesCard({ T, t0, tw }) {
  const p = clamp(MOTION.enter(t0 + 0.4, 1.1)(T), 0, 1);
  const drift = clamp(MOTION.draw(t0, 7)(T), 0, 1);
  const cw = clamp(MOTION.enter(t0 + 1.4, 1)(T), 0, 1);
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
      <Dim o={0.97 * clamp(MOTION.enter(t0, 0.7)(T), 0, 1)} color={C.ink} />
      <div style={{ position: 'absolute', left: '50%', top: '50%', opacity: 0.05, transform: 'translate(-50%,-50%) scale(' + (1.05 + drift * 0.18) + ')' }}>
        <Monogram h={320} color={C.cream} />
      </div>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <Monogram h={66} color={C.gold} style={{ opacity: p, marginBottom: 52 }} />
        <div style={{ fontFamily: SERIF, fontWeight: 400, textWrap: 'balance', fontSize: 116, color: C.white, textAlign: 'center', lineHeight: 1.04, ...rise(MOTION.enter(t0 + 0.6, 1)(T), 34) }}>
          The Life You're Building
        </div>
        <div style={{ width: 260 * cw, height: 3, background: C.gold, margin: '46px 0 34px' }} />
        <div style={{ ...caps(30, C.goldLite, 10), ...rise(MOTION.enter(t0 + 1.6, 0.9)(T), 20) }}>The Stewardship Blueprint</div>
        <div style={{ ...caps(21, 'rgba(244,241,234,0.5)', 6), marginTop: 42, opacity: clamp(MOTION.enter(t0 + 2.4, 0.9)(T), 0, 1) }}>{tw.url}</div>
      </div>
      {[[110, 110, 1, 1], [1810, 110, -1, 1], [110, 970, 1, -1], [1810, 970, -1, -1]].map(([x, y, sx, sy], i) => (
        <svg key={i} width={90} height={90} style={{ position: 'absolute', left: x - (sx > 0 ? 0 : 90), top: y - (sy > 0 ? 0 : 90), opacity: cw, transform: 'scale(' + sx + ',' + sy + ')' }}>
          <path d={'M0,0 L' + 76 * cw + ',0 M0,0 L0,' + 76 * cw} stroke={C.gold} strokeWidth={3} />
...
function Piece({ tw, scenes }) {
  ...
  const lt = beat('LowerThird'), fn = beat('Finished'), dw = beat('Drawer'), kb = beat('KeepBuilding'),
    st = beat('Steps'), pk = beat('PulseKeep'), lk = beat('Links'), ov = beat('Overflow'),
    pr = beat('Prayer'), sd = beat('SeriesCard');

  const capItems = [
    { at: lt.t0, text: 'You finished. Six weeks of meaningful work. I am proud of you, and I hope you recognize the work you put in.' },
    { at: fn.t0, until: fn.t0 + 0.1, text: '' },
    { at: dw.t0, text: 'A blueprint was never meant to sit in a drawer. So the question is not did I finish. It is how do I keep going.' },
    { at: kb.t0, until: kb.t0 + 0.1, text: '' },
    { at: st.t0, text: 'First, keep the accountability alive. Men often drift back when they go quiet.' },
    { at: pk.t0, text: 'Different seasons may change which pillar receives the most attention, but all four should remain visible.' },
    { at: lk.t0, until: lk.t0 + 0.1, text: '' },
    { at: ov.t0, text: 'That is what a well-built life does. It overflows to your family, your friends, your church, and your community.' },
    { at: pr.t0, until: pr.t0 + 0.1, text: '' },
    { at: sd.t0, until: sd.t0 + 0.1, text: '' },
  ];
  ...
  return (
    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }} data-screen-label={'t=' + Math.floor(T) + 's'}>
      ...
      <Beat from={fn.t0} to={fn.t1}><BFinished T={T} t0={fn.t0} /></Beat>
      <Beat from={dw.t0} to={dw.t1}><BDrawer T={T} t0={dw.t0} /></Beat>
      <Beat from={kb.t0} to={kb.t1}><BKeepBuilding T={T} t0={kb.t0} /></Beat>
      <Beat from={st.t0} to={st.t1}><BSteps T={T} t0={st.t0} /></Beat>
      <Beat from={pk.t0} to={pk.t1}><BPulseKeep T={T} t0={pk.t0} /></Beat>
      <Beat from={lk.t0} to={lk.t1}><BLinks T={T} t0={lk.t0} tw={tw} /></Beat>
      <Beat from={ov.t0} to={ov.t1}><BOverflow T={T} t0={ov.t0} /></Beat>
      <Beat from={pr.t0} to={pr.t1}><BPrayer T={T} t0={pr.t0} /></Beat>
      <Beat from={sd.t0} to={sd.t1}><BSeriesCard T={T} t0={sd.t0} tw={tw} /></Beat>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <TweaksPanel>
        ...
        <TweakText label="App URL" value={tw.appUrl} onChange={(v) => setTweak('appUrl', v)} />
        <TweakText label="URL" value={tw.url} onChange={(v) => setTweak('url', v)} />
      </TweaksPanel>
    </div>
  );
}
window.ClosingKit = ClosingKit;
