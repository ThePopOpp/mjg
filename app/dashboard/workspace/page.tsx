...
import { listDocuments, listFolders, listWorkspaces, DEFAULT_WORKSPACE_ID } from "@/lib/workspace/repository";
...
export default async function WorkspacePage({ searchParams }: { searchParams: Promise<{ ws?: string }> }) {
  ...
  const [{ mine, shared }, folders] = await Promise.all([listDocuments(profile.id, currentWorkspaceId), listFolders(profile.id, currentWorkspaceId)]);
