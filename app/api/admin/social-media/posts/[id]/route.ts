...
import { deletePost, updatePost } from "@/lib/social-media/data";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const m = error instanceof Error ? error.message : "Failed to update post.";
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const m = error instanceof Error ? error.message : "Failed to delete post.";
    return NextResponse.json({ error: m }, { status: errStatus(m) });
  }
}
